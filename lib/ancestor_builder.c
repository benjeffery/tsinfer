...
void
unpackbits(const uint8_t *restrict source, size_t len, int8_t *restrict dest)
{
    ...
    const allele_t *genotypes, uint8_t **ret_dest)
{
    ...
    uint8_t *dest
        = tsk_blkalloc_get(&self->main_allocator, self->encoded_genotypes_size);
...
ancestor_builder_add_site(ancestor_builder_t *self, double time, allele_t *genotypes)
{
    ...
    ret = ancestor_builder_store_genotypes(self, site_id, genotypes, &encoded_genotypes);
    if (ret != 0) {
        goto out;
    }
    self->num_sites++;
    ...
    if (avl_node == NULL) {
        ...
        map_elem->encoded_genotypes = encoded_genotypes;
